package S1.ArrayProblems;// Example of creating and using two-dimensional arrays
...
public class Array2Dexercise {
    ...
    public static String tableToString(int[][] table) {
        ...
        str.append(table.length).append(" rows by ").append(numCols).append(" cols\n     ");
        ...
        for (int row = 0; row < table.length; row++) {
            str.append(row).append(" | ");  // row index

            // include row
            for (int col = 0; col < table[row].length; col++) {
                str.append(table[row][col]).append(" ");
            }
            str.append("\n");  // end the row
        }
